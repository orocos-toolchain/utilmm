...
namespace utilmm
{
    ...
    void endian_swap_helper(const D data, D& buffer);

    template<> inline void endian_swap_helper<1, uint8_t>(const uint8_t data, uint8_t& buffer)
    { buffer = data; }
    template<> inline void endian_swap_helper<2, uint16_t>(const uint16_t data, uint16_t& buffer)
    { buffer = ((data >> 8) & 0xFF) | ((data << 8) & 0xFF00); }
    template<> inline void endian_swap_helper<4, uint32_t>(const uint32_t data, uint32_t& buffer)
    { buffer = ((data & 0xFF000000) >> 24) | ((data & 0x00FF0000) >> 8) | ((data & 0xFF) << 24) | ((data & 0xFF00) << 8); }
    template<> inline void endian_swap_helper<8, uint64_t>(const uint64_t data, uint64_t& buffer)
    { 
        const uint32_t 
        ...
        endian_swap_helper<4, uint32_t>( src_high, dst_low );
        endian_swap_helper<4, uint32_t>( src_low, dst_high );
    ...
    template<typename S>
    inline void endian_swap(const S data, S& buffer)
    { 
	typedef typename type_from_size<sizeof(S) * 8>::least T;
	endian_swap_helper<sizeof(S), T> (reinterpret_cast<const T&>(data), reinterpret_cast<T&>(buffer)); 
    }

    /* Returns the value of \c data with endianness swapped */
    template<typename S>
    inline S endian_swap(const S data)
    { S ret;
	endian_swap(data, ret);
	return ret;
    }
    ...
    { endian_swap<S>(source, dest); }

    template<typename S>
    inline S endian_native_to_network(const S source)
    { return endian_swap<S>(source); }
#else
